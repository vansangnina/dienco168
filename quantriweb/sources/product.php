...
function get_mans(){
	...
	$sql = "select sp_bc,ten_vi,id,stt,hienthi,id_list,id_cat,noibat,id_item,id_sub,sp_banchay,soluong,sp_khuyenmai from $where $limit";
	$d->query($sql);
...
function save_man(){
	...
	if($id){
		...
		if($_POST['khuyenmai_vi1']){
			$data['khuyenmai_vi1'] = implode('|',$_POST['khuyenmai_vi1']);
		}
...
function save_list(){
	...
	if($id){
		...
		if($quangcao = upload_image("quangcao", 'jpg|png|gif|JPG|jpeg|JPEG', _upload_product,$file_quangcao)){
			...
			$data['quangcao_thumb'] = create_thumb($data['quangcao'], 270, 415, _upload_product,$file_quangcao,1);		
			$d->setTable('product_list');
			$d->setWhere('id', $id);
			$d->select();
			if($d->num_rows()>0){
